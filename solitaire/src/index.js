...
const moveCardTo = (dest, i, card) => {
    ...
    if (dest === "finish" && location !== "finish") {
        audio.home.play();
        postMessage({
            state: GameState.MoveHome
        });
    }

    if (dest === "desk") {
        if (location === "finish") postMessage({
            state: GameState.DrawHome
        });
        if (location === "deal") postMessage({
            state: GameState.DrawDeal
        });
    }
};
...
const win = () => {
    ...
    const throwCard = (x, y) => {
        ...
        const particle = new Particle(
            ...
            x,
            y,
            Math.floor(Math.random() * 6 - 3) * 2, -Math.random() * 16
        );
    ...
    for (let i = 0; i < 4; i++) {
        const {
            left,
            top
        } = state.finish[i].el.getBoundingClientRect();
